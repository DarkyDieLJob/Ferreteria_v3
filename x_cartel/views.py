    ...
    def get_context_data(self, request, *args, **kwargs):
        ...
        try:
            if cajon_id is not None:
                ...
                if cajon_id is not None:
                    try:
                        cartel, created = CartelesCajon.objects.get_or_create(
                            id=cajon_id
                        )
                        context["cartel"] = cartel
                    except Exception as e:
...
class CrearCartelitoView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            data = {
                "descripcion": cartelito.descripcion,
                "habilitado": cartelito.habilitado,
            }
    ...
    def post(self, request, *args, **kwargs):
        ...
        item = Item.objects.get(id=item_id)
        cartelito, created = Cartelitos.objects.get_or_create(
            item=item, defaults={"descripcion": item.descripcion}
        )
        if not created:
            cartelito.habilitado = not cartelito.habilitado
            if (
                descripcion is not None
            ):  # Aquí verificamos si se proporcionó una descripción
                cartelito.descripcion = (
                    descripcion  # Si se proporcionó, actualizamos la descripción
                )
            cartelito.save()
        return JsonResponse(
            {"descripcion": cartelito.descripcion, "habilitado": cartelito.habilitado}
        )
