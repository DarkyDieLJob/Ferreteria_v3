        ...
        with transaction.atomic():
            for row in lote:
                ...
                if any(
                    ...
                    for key, value in row.items()
                    if key not in ["sub_carpeta", "sub_titulo"]
                ):
                    continue
                try:
                    # Recuperar o crear una instancia de Sub_Carpeta
                ...
                try:
                    item, created = Item.objects.get_or_create(
                        codigo=row["codigo"], defaults=row
                    )
                    if not created:
                        for key, value in row.items():
                            setattr(item, key, value)
            ...
            Item.objects.bulk_update(
                ...
                [
                    ...
                    if field.name != Item._meta.pk.name
                ],
            )
...
def apply_custom_round(batch_size=10000):
    ...
    while offset < total_items:
        ...
        Item.objects.bulk_update(
            batch, ["final", "final_efectivo", "final_rollo", "final_rollo_efectivo"]
        )
        offset += batch_size
