...
def obtener_metodos_pago(request):
    ...
    data = [
        {"id": metodo_pago.id, "display": metodo_pago.display}
        ...
        try:
            ...
            for boleta in boletas_json:
                try:
                    ...
                    if respuesta and isinstance(respuesta, list) and len(respuesta) > 0:
                        ...
                        tipo_cbte = (
                            boleta.get("printTicket", {})
                            .get("cabecera", {})
                            .get("tipo_cbte")
                        )
                        logger.info(
                            f"Boleta generada: Tipo={tipo_cbte}, Número={numero_cbte}"
                        )
                    else:
                        logger.warning(
                            "Formato de respuesta inesperado o vacío de websocket."
                        )

                except asyncio.CancelledError as e:
                    logger.warning(
                        "La conexión websocket fue cancelada.", exc_info=True
                    )
                    return HttpResponse("Ticket cancelado", status=500)
                except asyncio.TimeoutError as e:
                    logger.warning(
                        "Timeout esperando respuesta del websocket.", exc_info=True
                    )
                    return HttpResponse("Tiempo de respuesta excedido", status=500)
                except ValueError as e:
                    logger.error(
                        f"Error al convertir número de comprobante a entero: {numero_cbte_str}",
                        exc_info=True,
                    )
                    return HttpResponse(
                        "Error en formato de número de comprobante", status=500
                    )
                except Exception as e:
                    logger.error(
                        "Error inesperado durante la comunicación websocket o procesamiento de respuesta.",
                        exc_info=True,
                    )
            ...
            if transaccion:
                try:
                    ...
                    transaccion.save()
                    logger.info(
                        f"Transacción ID {transaccion.id} actualizada con Tipo Cbte: {tipo_cbte}, Número Cbte: {numero_cbte}"
                    )
                except Exception as e:
                    logger.warning(
                        f"No se pudo asignar tipo_cbte o numero_cbte a la transacción {transaccion.id}.",
                        exc_info=True,
                    )
            else:
...
def agregar_articulo_sin_registro(request):
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Error al convertir cantidad ({cantidad_str}) o precio ({precio_str}) a número."
            )
            return JsonResponse({"error": "Cantidad o precio inválido"}, status=400)
        except Carrito.DoesNotExist:
            logger.warning(
                f"Intento de agregar artículo a carrito inexistente ID: {carrito_id}"
            )
            return JsonResponse({"error": "Carrito no encontrado"}, status=404)

        # Handle potential duplicate descriptions within the same cart
        contador = ArticuloSinRegistro.objects.filter(
            carrito=carrito, descripcion__iexact=descripcion
        ).count()
        ...
        if contador > 0:
            ...
            logger.debug(
                f"Descripción duplicada encontrada. Nueva descripción: {descripcion_final}"
            )
        ...
        if created:
            logger.info(
                f"Artículo sin registro '{descripcion_final}' creado en carrito {carrito_id}."
            )
        else:
            logger.info(
                f"Artículo sin registro '{descripcion_final}' actualizado en carrito {carrito_id}."
            )

        return JsonResponse(
            {"articulo_sin_registro": articulo_sin_registro.id},
            status=201 if created else 200,
        )

    except json.JSONDecodeError:
        logger.error(
            "Error al decodificar JSON en agregar_articulo_sin_registro.", exc_info=True
        )
        return JsonResponse({"error": "JSON inválido"}, status=400)
    except Exception as e:
        logger.error(
            "Error inesperado en agregar_articulo_sin_registro.", exc_info=True
        )
...
def eliminar_articulo(request):
    ...
    try:
        ...
        try:
            articulo = Articulo.objects.select_related("item").get(
                id=articulo_id
            )  # Optimize query
        except Articulo.DoesNotExist:
            logger.warning(
                f"Intento de eliminar artículo inexistente ID: {articulo_id}"
            )
        ...
        try:
            ...
            pedido.save()
            logger.info(
                f"Actualizada cantidad en Lista_Pedidos para item {item_relacionado.id}. Nueva cantidad: {pedido.cantidad}"
            )
        except Lista_Pedidos.DoesNotExist:
            logger.info(
                f"No se encontró Lista_Pedidos para item {item_relacionado.id}. No se requiere actualización."
            )
        except Exception as e:
            # Catching broad exception here might hide issues, be more specific if possible
            logger.error(
                f"Error actualizando Lista_Pedidos para item {item_relacionado.id}",
                exc_info=True,
            )
        ...
        articulo.delete()
        logger.info(
            f"Artículo '{descripcion_articulo}' (ID: {articulo_id}) eliminado del carrito."
        )
...
def actualizar_cantidad_articulo(request):
    ...
    try:
        ...
        logger.info(
            f"Solicitud para actualizar cantidad del artículo ID: {articulo_id} a: {cantidad_nueva_str}"
        )

        if not articulo_id or cantidad_nueva_str is None:
            logger.warning(
                "ID de artículo o nueva cantidad faltante en solicitud de actualización."
            )
        ...
        try:
            ...
            if cantidad_nueva < 0:  # Add validation
                logger.warning(
                    f"Intento de actualizar artículo ID {articulo_id} con cantidad negativa: {cantidad_nueva}"
                )
                return JsonResponse(
                    {"error": "La cantidad no puede ser negativa"}, status=400
                )
        except ValueError:
            logger.warning(
                f"Error al convertir nueva cantidad '{cantidad_nueva_str}' a número para artículo ID {articulo_id}."
            )
        ...
        try:
            ...
            logger.warning(
                f"Intento de actualizar cantidad de artículo inexistente ID: {articulo_id}"
            )
            return JsonResponse({"error": "Artículo no encontrado"}, status=404)
        ...
        try:
            ...
            pedido.save()
            logger.info(
                f"Actualizada cantidad en Lista_Pedidos para item {articulo.item.id} por diferencia {diferencia_cantidad}. Nueva cantidad: {pedido.cantidad}"
            )
        except Lista_Pedidos.DoesNotExist:
            logger.info(
                f"No se encontró Lista_Pedidos para item {articulo.item.id}. No se requiere actualización."
            )
        except Exception as e:
            logger.error(
                f"Error actualizando Lista_Pedidos para item {articulo.item.id}",
                exc_info=True,
            )
        ...
        articulo.save()
        logger.info(
            f"Cantidad del artículo '{articulo.item.descripcion}' (ID: {articulo_id}) actualizada de {cantidad_anterior} a {cantidad_nueva}."
        )
        ...
        logger.error(
            "Error al decodificar JSON en actualizar_cantidad_articulo.", exc_info=True
        )
        return JsonResponse({"error": "JSON inválido"}, status=400)
    except Exception as e:
...
def eliminar_articulo_sin_registro(request):
    ...
    try:
        ...
        logger.info(
            f"Solicitud para eliminar artículo sin registro ID: {articulo_sr_id}"
        )

        if not articulo_sr_id:
            logger.warning(
                "ID de artículo sin registro faltante en solicitud de eliminación."
            )
            return JsonResponse(
                {"error": "ID de artículo sin registro requerido"}, status=400
            )
        ...
        try:
            ...
            logger.warning(
                f"Intento de eliminar artículo sin registro inexistente ID: {articulo_sr_id}"
            )
            return JsonResponse(
                {"error": "Artículo sin registro no encontrado"}, status=404
            )

        descripcion_articulo = (
            articulo_sin_registro.descripcion
        )  # Get description before deleting
        articulo_sin_registro.delete()
        logger.info(
            f"Artículo sin registro '{descripcion_articulo}' (ID: {articulo_sr_id}) eliminado."
        )
        ...
        logger.error(
            "Error al decodificar JSON en eliminar_articulo_sin_registro.",
            exc_info=True,
        )
        return JsonResponse({"error": "JSON inválido"}, status=400)
    except Exception as e:
        logger.error(
            "Error inesperado en eliminar_articulo_sin_registro.", exc_info=True
        )
        return JsonResponse({"error": "Error interno del servidor"}, status=500)
...
def actualizar_cantidad_articulo_sin_registro(request):
    ...
    try:
        ...
        cantidad_nueva_str = data.get("cantidad")
        logger.info(
            f"Solicitud para actualizar cantidad del artículo sin registro ID: {articulo_sr_id} a: {cantidad_nueva_str}"
        )

        if not articulo_sr_id or cantidad_nueva_str is None:
            logger.warning(
                "ID de artículo sin registro o nueva cantidad faltante en solicitud de actualización."
            )
            return JsonResponse({"error": "ID y cantidad requeridos"}, status=400)
        ...
        try:
            ...
            if cantidad_nueva < 0:  # Add validation
                logger.warning(
                    f"Intento de actualizar artículo sin registro ID {articulo_sr_id} con cantidad negativa: {cantidad_nueva}"
                )
                return JsonResponse(
                    {"error": "La cantidad no puede ser negativa"}, status=400
                )
            ...
            logger.warning(
                f"Error al convertir nueva cantidad '{cantidad_nueva_str}' a número para artículo sin registro ID {articulo_sr_id}."
            )
            return JsonResponse({"error": "Cantidad inválida"}, status=400)
        ...
        try:
            ...
            logger.warning(
                f"Intento de actualizar cantidad de artículo sin registro inexistente ID: {articulo_sr_id}"
            )
            return JsonResponse(
                {"error": "Artículo sin registro no encontrado"}, status=404
            )
        ...
        logger.info(
            f"Cantidad del artículo sin registro '{articulo_sin_registro.descripcion}' (ID: {articulo_sr_id}) actualizada de {cantidad_anterior} a {cantidad_nueva}."
        )
        ...
        logger.error(
            "Error al decodificar JSON en actualizar_cantidad_articulo_sin_registro.",
            exc_info=True,
        )
        return JsonResponse({"error": "JSON inválido"}, status=400)
    except Exception as e:
        logger.error(
            "Error inesperado en actualizar_cantidad_articulo_sin_registro.",
            exc_info=True,
        )
        return JsonResponse({"error": "Error interno del servidor"}, status=500)
...
class FacturacionMensual(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        logger.debug(
            f"Datos agrupados por fecha para facturación mensual: {datos_agrupados}"
        )
...
class Facturacion(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            logger.warning(
                f"Fecha inválida recibida: año={year}, mes={month}, día={day}. Usando fecha actual."
            )
        ...
        context["fecha_actual"] = (
            fecha_seleccionada  # Pass date to template for display
        )
        ...
        for metodo in metodos_pago:
            ...
            )  # Filter the already filtered queryset
            total_sum = (
                ventas.aggregate(Sum("total"))["total__sum"] or 0
...
class CierreZVieW(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        context["cierres_fiscales"] = CierreZ.objects.order_by(
            "-fecha"
        )  # Show newest first
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.info(
                f"Respuesta del websocket (tipo {type(respuesta_ws_dict)}) para Cierre Z: {respuesta_ws_dict}"
            )
            ...
            try:
                ...
                logger.debug(
                    f"Usando respuesta parseada directamente (tipo {type(data)}): {data}"
                )
                ...
                if (
                    ...
                    if rta_interno_lista:
                        ...
                        if (
                            isinstance(rta_dict_wrapper, dict)
                            and "rta" in rta_dict_wrapper
                        ):
                            rta = rta_dict_wrapper.get(
                                "rta"
                            )  # Diccionario con los datos del cierre
                            if rta and isinstance(rta, dict):
                                ...
                                cierre_z_obj = CierreZ(
                                    RESERVADO_SIEMPRE_CERO=int(
                                        rta.get("RESERVADO_SIEMPRE_CERO", 0)
                                    ),
                                    cant_doc_fiscales=int(
                                        rta.get("cant_doc_fiscales", 0)
                                    ),
                                    cant_doc_nofiscales_homologados=int(
                                        rta.get("cant_doc_nofiscales_homologados", 0)
                                    ),
                                    ultima_nc_a=int(rta.get("ultima_nc_a", 0)),
                                    ultima_nc_b=int(rta.get("ultima_nc_b", 0)),
                                    monto_percepciones=float(
                                        rta.get("monto_percepciones", 0.0)
                                    ),
                                    monto_percepciones_nc=float(
                                        rta.get("monto_percepciones_nc", 0.0)
                                    ),
                                    monto_credito_nc=float(
                                        rta.get("monto_credito_nc", 0.0)
                                    ),
                                    ultimo_doc_a=int(rta.get("ultimo_doc_a", 0)),
                                    ultimo_doc_b=int(rta.get("ultimo_doc_b", 0)),
                                    zeta_numero=rta.get("zeta_numero", "N/A"),
                                    monto_imp_internos_nc=float(
                                        rta.get("monto_imp_internos_nc", 0.0)
                                    ),
                                    cant_doc_fiscales_cancelados=int(
                                        rta.get("cant_doc_fiscales_cancelados", 0)
                                    ),
                                    monto_iva_doc_fiscal=float(
                                        rta.get("monto_iva_doc_fiscal", 0.0)
                                    ),
                                    monto_imp_internos=float(
                                        rta.get("monto_imp_internos", 0.0)
                                    ),
                                    ...
                                    monto_ventas_doc_fiscal=float(
                                        rta.get("monto_ventas_doc_fiscal", 0.0)
                                    ),
                                    cant_doc_nofiscales=int(
                                        rta.get("cant_doc_nofiscales", 0)
                                    ),
                                )
                                cierre_z_obj.save()
                                logger.info(
                                    f"Nuevo Cierre Z guardado con ID: {cierre_z_obj.id} y número Z: {cierre_z_obj.zeta_numero}"
                                )
                                ...
                                try:
                                    ...
                                    logger.info(
                                        "Cola de tareas ejecutada después del Cierre Z."
                                    )
                                    return redirect(
                                        "/vista_cierre_z/"
                                    )  # Redirigir a la vista de Cierre Z después de guardar
                                    ...
                                    logger.error(
                                        "Error al ejecutar la cola de tareas después del Cierre Z.",
                                        exc_info=True,
                                    )

                            else:
                                logger.error(
                                    "La clave 'rta' interna no contiene un diccionario válido o está vacía."
                                )
                                context["error_procesamiento"] = (
                                    "Formato de respuesta interna inválido (no es diccionario)."
                                )
                        else:
                            logger.error(
                                "El primer elemento de la lista 'rta' no es un diccionario o no contiene la clave 'rta'."
                            )
                            context["error_procesamiento"] = (
                                "Formato de respuesta interna inválido (wrapper)."
                            )
                    else:
                        logger.error("La lista 'rta' está vacía.")
                        context["error_procesamiento"] = (
                            "Formato de respuesta inválido (lista rta vacía)."
                        )
                else:
                    logger.error(
                        "Formato inesperado en la respuesta principal del Cierre Z (no es dict, falta 'rta', etc.)."
                    )
                    context["error_procesamiento"] = (
                        "Formato de respuesta principal inválido."
                    )
            ...
            # Es buena idea capturar KeyError por si falta alguna clave esperada en el diccionario 'rta'.
                ...
                logger.error(
                    f"Error de clave al procesar la respuesta del Cierre Z: Falta la clave {ke}",
                    exc_info=True,
                )
                ...
                logger.error(
                    f"Error de valor al convertir datos del Cierre Z: {ve}",
                    exc_info=True,
                )
                context["error_procesamiento"] = (
                    f"Error de formato en un valor numérico: {ve}"
                )
            except Exception as e:  # Captura general para otros errores (DB, etc.)
                logger.error(
                    f"Error general al procesar o guardar el Cierre Z: {e}",
                    exc_info=True,
                )
            ...
            context["error_websocket"] = (
                "Tiempo de espera agotado conectando con impresora."
            )
        except Exception as ws_err:
            logger.error(
                "Error durante la comunicación websocket para Cierre Z.", exc_info=True
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            try:
                ...
                logger.info(
                    f"Cliente '{cliente.razon_social}' (ID: {cliente.id}) guardado exitosamente."
                )
...
def consulta_impresora_fiscal_generica(request):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            logger.info(
                f"Respuesta cruda del websocket para consulta genérica: {respuesta_ws_str}"
            )
            ...
            try:
                ...
                logger.warning(
                    "Respuesta no es JSON, intentando parsear como Python literal."
                )
                try:
                    import ast
                    ...
                    logger.warning(
                        "Respuesta no pudo ser parseada como JSON ni Python literal. Devolviendo cruda."
                    )
            ...
            logger.warning(
                "Timeout esperando respuesta del websocket para consulta genérica."
            )
            return JsonResponse(
                {"error": "Tiempo de espera agotado"}, status=504
            )  # Gateway Timeout
        except Exception as e:
            logger.error(
                f"Error durante la comunicación websocket para consulta genérica.",
                exc_info=True,
            )
            return JsonResponse(
                {"error": f"Error al ejecutar el comando: {e}"}, status=500
            )
    else:
        logger.warning(
            f"Método {request.method} no permitido para consulta_impresora_fiscal_generica."
        )
