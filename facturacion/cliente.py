...
from core_config.settings import (
    IP_BEW_SOCKET,
)  # Ensure this setting is correctly configured
...
async def conectar_a_websocket(data):
    ...
    logger.debug(
        f"Enviando datos: {data}"
    )  # Log the data being sent (consider redacting sensitive info if necessary)
    ...
    try:
        ...
        async with websockets.connect(uri) as websocket:
            ...
            logger.debug(
                f"Respuesta cruda recibida del websocket (tipo: {type(respuesta)})"
            )
            ...
            if isinstance(
                respuesta, (str, bytes)
            ):  # Handle bytes too, decode if necessary
                if isinstance(respuesta, bytes):
                    try:
                        respuesta_str = respuesta.decode("utf-8")
                    except UnicodeDecodeError as ude:
                        logger.error(
                            f"Error al decodificar respuesta bytes como UTF-8: {ude}",
                            exc_info=True,
                        )
                        return {
                            "error": "Respuesta bytes no es UTF-8 válido",
                            "respuesta_bytes": respuesta.hex(),
                ...
                try:
                    ...
                    logger.debug(
                        f"Respuesta parseada como JSON (tipo: {type(respuesta_json)}): {respuesta_json}"
                    )
                    ...
                    logger.warning(
                        f"Error al decodificar respuesta como JSON: {e}. Respuesta recibida: {respuesta_str}",
                        exc_info=False,
                    )
                    return {
                ...
                logger.warning(
                    f"Respuesta del websocket no es una cadena o bytes (tipo: {type(respuesta)})."
                )
                return {
                    "error": "Respuesta no es una cadena o bytes",
        ...
        logger.warning(
            f"Conexión websocket cerrada: Código={e.code}, Razón='{e.reason}'",
            exc_info=False,
        )
        ...
        logger.error(
            f"Conexión websocket rechazada para URI: '{uri}'. ¿Servidor corriendo?",
            exc_info=True,
        )
        ...
        logger.error(
            f"Error inesperado durante la comunicación websocket con {uri}.",
            exc_info=True,
        )
