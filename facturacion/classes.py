...
class FormasPago:
    ...
    def __init__(self, transaccion):
        ...
        try:
            ...
            logger.debug(
                f"FormasPago inicializado: ds='{self.ds}', importe={self.importe}"
            )
        except AttributeError as e:
            logger.error(
                f"Error al inicializar FormasPago para transacción {transaccion.id}: {e}",
                exc_info=True,
            )
    ...
    def get_importe(self):
        ...
        try:
            ...
            logger.warning(
                f"No se pudo convertir el importe '{self.importe}' a float en FormasPago: {e}"
            )
    ...
    def get_formas_pago_json(self, cant=1):
        ...
        if cant <= 0:
            logger.warning(
                f"Cantidad inválida ({cant}) en get_formas_pago_json. Usando 1."
            )
            cant = 1
    ...
    def __init__(self, cliente_id=None):
        ...
        if cliente_id is not None:
            try:
                ...
                logger.warning(
                    f"TicketCabecera.__init__ - ID de cliente inválido: '{cliente_id}'. Usando consumidor final."
                )

        if (
            valid_id and cliente_id_int != 1
        ):  # Assuming ID 1 is Consumidor Final default
            try:
                self.cliente = Cliente.objects.get(id=cliente_id_int)
                logger.debug(
                    f"TicketCabecera.__init__ - Cliente encontrado: {self.cliente}"
                )
                ...
                if responsabilidad == "EXENTO":
                    ...
                    self.tipo_cbte = "FB"
                    logger.debug(
                        f"Cliente {cliente_id_int} con responsabilidad '{responsabilidad}', usando tipo_cbte FB."
                    )
                ...
                logger.debug(
                    f"Cabecera configurada para cliente ID {cliente_id_int}: tipo_cbte={self.tipo_cbte}, tipo_resp={self.tipo_responsable}"
                )

            except Cliente.DoesNotExist:
                logger.warning(
                    f"TicketCabecera.__init__ - Cliente con ID {cliente_id_int} no encontrado. Usando consumidor final."
                )
                # Reset to default consumer final values if client not found
                self._set_consumidor_final_defaults()
            except Exception as e:
                logger.error(
                    f"Error al obtener datos del cliente ID {cliente_id_int}: {e}",
                    exc_info=True,
                )
                self._set_consumidor_final_defaults()
        else:
...
class TicketItem:
    ...
    def __init__(self, data):
        ...
        self.alic_iva = float(
            dic_item.get("alic_iva", 21.0)
        )  # Assuming 21 is default IVA
        self.importe = float(dic_item.get("importe", 0.0))
        self.importe_efectivo = float(
            dic_item.get("importe_efectivo", self.importe)
        )  # Default efectivo price to normal price if missing
    ...
    def set_div_cant(self, cantidad):
        ...
        if cantidad <= 0:
            logger.warning(
                f"Intento de dividir cantidad por cero o negativo ({cantidad}) en set_div_cant para item '{self.ds}'. No se modifica la cantidad."
            )
            return
        original_qty = self.qty
        self.qty = round(
            self.qty / cantidad, 8
        )  # Use high precision for division result
        logger.debug(
            f"Item '{self.ds}': cantidad dividida por {cantidad}. Nueva qty: {self.qty} (original: {original_qty})"
        )
    ...
    def get_item_json(self, efectivo=False, boleta_a=False):
        ...
        if boleta_a:
            ...
            precio_sin_iva = round(
                float(importe_base) / 1.21, 4
            )  # Use higher precision for intermediate calc
            logger.debug(
                f"Item '{self.ds}': Boleta A detectada. Calculando precio sin IVA: {precio_sin_iva} (desde {importe_base})"
            )
        ...
        item_json = {
            ...
            "ds": (
                self.ds[:30] if len(self.ds) > 30 else self.ds
            ),  # Truncate description if too long (adjust limit as needed)
            "importe": float(importe_final_rounded),  # Ensure float
            "qty": float(
                round(self.qty, 3)
            ),  # Round quantity for JSON (adjust precision as needed)
...
class TicketFactura:
    ...
    def __init__(self, transaccion):
        ...
        logger.debug(
            f"TicketFactura.__init__ - ID Cliente obtenido de transacción: {cliente_id}"
        )
        ...
        if importe_total > MONTO_LIMITE_TICKET and MONTO_LIMITE_TICKET > 0:
            ...
            logger.info(
                f"Importe total ({importe_total}) excede límite ({MONTO_LIMITE_TICKET}). Se generarán {self.cantidad} tickets."
            )
        else:
            self.cantidad = 1
            logger.debug(
                f"Importe total ({importe_total}) dentro del límite. Se generará 1 ticket."
            )
        ...
        for articulo_vendido in self.transaccion.articulos_vendidos.all():
            try:
                item_data = (
                    articulo_vendido.get_item()
                )  # Assumes this method returns the dict needed by TicketItem
                if item_data:
                    ticket_item = TicketItem(item_data)
                    self.items.append(ticket_item)
                else:
                    logger.warning(
                        f"ArticuloVendido ID {articulo_vendido.id} devolvió datos vacíos en get_item()."
                    )
            except Exception as e:
                logger.error(
                    f"Error al procesar ArticuloVendido ID {articulo_vendido.id}: {e}",
                    exc_info=True,
                )
    ...
    def get_items_json(self, efectivo, boleta_a):
        ...
        logger.debug(
            f"Generando JSON de items: efectivo={efectivo}, boleta_a={boleta_a}"
        )
        for ticket_item in self.items:
            items_json_list["items"].append(
                ticket_item.get_item_json(efectivo=efectivo, boleta_a=boleta_a)
            )
    ...
    def get_ticket_json(self):
        ...
        for i in range(self.cantidad):
            ...
            ticket_json["printTicket"].update(
                self.get_items_json(es_efectivo, es_boleta_a)
            )
            ticket_json["printTicket"].update(
                self.formas_pago.get_formas_pago_json(self.cantidad)
            )
        ...
        logger.debug(
            f"TicketFactura.get_ticket_json - JSON final generado: {json_list}"
        )
...
class ComandoFiscal:
    ...
    MONTO_LIMITE_TICKET_CMD = (
        25000.00  # Configurable? Example limit for command-based printers
    )

    def __init__(
        self, carrito_id=None, id_cliente=None, tipo_pago=None, monto_abonado="0"
    ):
        logger.info(
            f"Inicializando ComandoFiscal: carrito_id={carrito_id}, id_cliente={id_cliente}, tipo_pago='{tipo_pago}'"
        )
        ...
        try:
            ...
            self.articulos_sin_registro = list(
                self.carrito.articulosinregistro_set.all()
            )
            logger.debug(
                f"Carrito ID {carrito_id} cargado con {len(self.articulos)} artículos y {len(self.articulos_sin_registro)} artículos sin registro."
            )
        ...
        if id_cliente:
            try:
                ...
                if (
                    self.cliente.responsabilidad_iva == "I"
                ):  # Assuming 'I' means RESPONSABLE_INSCRIPTO
                    self.tipo_boleta = "A"
                logger.debug(
                    f"Cliente ID {id_cliente} encontrado. Tipo de boleta: {self.tipo_boleta}"
                )
            except Cliente.DoesNotExist:
                logger.warning(
                    f"Cliente ID {id_cliente} no encontrado. Usando boleta tipo B."
                )
            except Exception as e:
                logger.error(
                    f"Error al obtener cliente ID {id_cliente}: {e}", exc_info=True
                )
        ...
        self.pago_efectivo = (
            isinstance(self.tipo_pago, str)
            and self.tipo_pago.lower() == "efectivo con ticket"
        )
        ...
        try:
            ...
            logger.info(
                f"Creada nueva Boleta ID: {self.boleta.id}, Tipo: {self.tipo_boleta}"
            )
        except Exception as e:
            logger.error(
                "Error al crear el objeto Boleta en la base de datos.", exc_info=True
            )
        ...
        total_calculado = (
            self._calcular_total_carrito()
        )  # Calculate total based on payment type

        if (
            total_calculado > self.MONTO_LIMITE_TICKET_CMD
            and self.MONTO_LIMITE_TICKET_CMD > 0
        ):
            self.cantidad_tickets = math.ceil(
                total_calculado / self.MONTO_LIMITE_TICKET_CMD
            )
            self.factor = 1.0 / self.cantidad_tickets
            logger.info(
                f"Total calculado ({total_calculado}) excede límite ({self.MONTO_LIMITE_TICKET_CMD}). Se generarán {self.cantidad_tickets} tickets (factor: {self.factor})."
            )
        else:
            logger.debug(
                f"Total calculado ({total_calculado}) dentro del límite. Se generará 1 ticket."
            )

        # Set dynamic commands based on init params
        self.CMD_OPEN_FISCAL_RECEIPT = (
            f"@\x1c{self.tipo_boleta}\x1cS"  # Command to open receipt
        )
    ...
    def _calcular_total_carrito(self):
        ...
        logger.debug(
            f"Calculando total del carrito usando precio: '{precio_field if self.pago_efectivo else 'item__final'}'"
        )
        ...
        for art in self.articulos:
            try:
                ...
                logger.warning(
                    f"Artículo {art.id} o su item relacionado no tiene el campo de precio esperado."
                )
            except (ValueError, TypeError):
                logger.warning(
                    f"Error al convertir precio o cantidad para artículo {art.id}"
                )
        ...
        for art_sr in self.articulos_sin_registro:
            try:
                ...
                logger.warning(
                    f"Error al convertir precio o cantidad para artículo sin registro {art_sr.id}"
                )
    ...
    def _add_comando(self, comando_str, orden):
        ...
        if not isinstance(comando_str, str) or not comando_str:
            logger.warning(
                f"Intento de agregar comando inválido (orden {orden}): {comando_str}"
            )
            return
        try:
            ...
            OrdenComando.objects.create(
                boleta=self.boleta, comando=comando, orden=orden
            )
            ...
            logger.error(
                f"Error al agregar comando '{comando_str}' (orden {orden}) a la Boleta {self.boleta.id}.",
                exc_info=True,
            )
    ...
    def _str_history_capacity(self, orden):
        self._add_comando(self.CMD_HISTORY_CAPACITY, orden)

    def _informe_z(self, orden):
        self._add_comando(self.CMD_INFORME_Z, orden)

    def _cierre_fiscal_z(self, orden):
        self._add_comando(self.CMD_CIERRE_FISCAL_Z, orden)

    # ... (other specific command methods like _daily_close_by_date, etc.) ...
    def _get_working_memory(self, orden):
        self._add_comando(self.CMD_GET_WORKING_MEMORY, orden)

    def _send_first_IVA(self, orden):
        self._add_comando(self.CMD_SENT_FIRST_IVA, orden)

    def _next_IVA_transmission(self, orden):
        self._add_comando(self.CMD_NEXT_IVA_TRANSMISSION, orden)

    def _open_fiscal_receipt(self, orden):
        self._add_comando(self.CMD_OPEN_FISCAL_RECEIPT, orden)

    def _print_fiscal_text(self, texto, orden):
        self._add_comando(f"A\x1c{texto}\x1c0", orden)

    def _print_line_item(self, descripcion, cantidad, precio, orden):
        # Ensure values are formatted correctly for the command string
        desc_trunc = (
            descripcion[:20] if len(descripcion) > 20 else descripcion
        )  # Printer char limit
    ...
    def _last_item_discount(self, texto, cantidad, orden):
        self._add_comando(f"U\x1c{texto}\x1c{cantidad:.2f}\x1cm\x1c0\x1cT", orden)

    def _last_item_recargo(self, texto, cantidad, orden):
        self._add_comando(f"U\x1c{texto}\x1c{cantidad:.2f}\x1cM\x1c0\x1cT", orden)

    def _general_discount(self, texto, cantidad, orden):
        self._add_comando(f"T\x1c{texto}\x1c{cantidad:.2f}\x1cm\x1c0\x1cT", orden)

    # ... (other commands) ...
    def _sub_total(self, orden):
        self._add_comando(self.CMD_SUB_TOTAL, orden)
    ...
    def _total_tender(self, orden):
        ...
        monto_pago = (
            self.monto_abonado
            if self.pago_efectivo
            else self.sub_total_calculado_para_ticket
        )
        monto_pago_str = f"{float(monto_pago):.2f}"
        logger.debug(
            f"ComandoFiscal._total_tender - Monto para comando D: {monto_pago_str} (Subtotal Ticket: {self.sub_total_calculado_para_ticket}, Pago Efectivo: {self.pago_efectivo}, Monto Abonado Original: {self.monto_abonado})"
        )
        # Ensure tipo_pago doesn't exceed printer limits
        tipo_pago_trunc = (
            self.tipo_pago[:20] if len(self.tipo_pago) > 20 else self.tipo_pago
        )
    ...
    def _close_fiscal_receipt(self, orden):
        self._add_comando(self.CMD_CLOSE_FISCAL_RECEIPT, orden)

    def _set_customer_data(self, orden):
        if self.cliente:
            # Clean and format client data for the command
            razon_social = (self.cliente.razon_social or "")[
                :40
            ]  # Check printer limits
            cuit_dni = (self.cliente.cuit_dni or "").replace("-", "")[:20]
            resp_iva = (
                self.cliente.responsabilidad_iva or "C"
            )  # Default to Consumidor Final IVA type?
    ...
    def _set_fantasy_name(self, texto, orden):
        self._add_comando(f"{self.CMD_SET_FANTASY_NAME_BASE}{texto}", orden)

    def _set_fantasy_name_barra(self, i, orden):
        self._set_fantasy_name(f"{i+1}/{self.cantidad_tickets}", orden)

    def _get_status(self, orden):
        self._add_comando(self.CMD_GET_STATUS, orden)
    ...
    def set_encabezado(self, ticket_index):
        ...
        logger.info(
            f"Configurando encabezado para ticket {ticket_index + 1}/{self.cantidad_tickets}."
        )
        self._get_status(self.orden)
        self.orden += 1  # Start with status check
        # Set Fantasy Name (e.g., Store Name) - Do this only once? Or per ticket? Depends on printer.
        self._set_fantasy_name(self.DEFAULT_FANTASY_NAME, self.orden)
        self.orden += 1
        # Add ticket sequence if splitting
        if self.cantidad_tickets > 1:
            self._set_fantasy_name_barra(ticket_index, self.orden)
            self.orden += 1
    ...
    def set_articulos(self):
        ...
        for articulo in self.articulos + self.articulos_sin_registro:
            ...
            if isinstance(articulo, Articulo):
                ...
                precio = float(
                    articulo.item.final_efectivo
                    if self.pago_efectivo
                    else articulo.item.final
                )
        ...
        logger.debug(
            f"Subtotal calculado para set_articulos: {self.sub_total_calculado_para_ticket}"
        )
    ...
    def set_articulos_desborde(self):
        ...
        self.sub_total_calculado_para_ticket = (
            0.0  # Reset subtotal for the current ticket being generated
        )
        for articulo in self.articulos + self.articulos_sin_registro:
            logger.debug(
                f"ComandoFiscal.set_articulos_desborde - Procesando: {articulo}"
            )
            precio = 0.0
            cantidad = 0.0
            descripcion = "N/A"
            ...
            try:
                if isinstance(articulo, Articulo):
                    ...
                    logger.debug(
                        f"  -> Artículo reg: desc='{descripcion}', precio={precio}"
                    )
                else:  # ArticuloSinRegistro
                    descripcion = articulo.descripcion
                    precio = float(articulo.precio)
                    logger.debug(
                        f"  -> Artículo s/reg: desc='{descripcion}', precio={precio}"
                    )

                cantidad = float(articulo.cantidad)
                logger.debug(
                    f"  -> Cantidad original: {cantidad}, Factor: {self.factor}"
                )

                cantidad_aplicar = round(cantidad * self.factor, 10)  # Apply factor
                precio_aplicar = round(
                    precio, 4
                )  # Use original price, quantity is adjusted

                # Add command using adjusted quantity and original price
                self._print_line_item(
                    descripcion, cantidad_aplicar, precio_aplicar, self.orden
                )
                self.orden += 1
                self.sub_total_calculado_para_ticket += (
                    precio_aplicar * cantidad_aplicar
                )
                ...
                logger.warning(
                    f"Error de valor/tipo procesando item {articulo.id}: {ve}"
                )
            except Exception as e:
                logger.error(
                    f"Error inesperado procesando item {articulo.id} en set_articulos_desborde",
                    exc_info=True,
                )

        # Round the final subtotal for the ticket
        self.sub_total_calculado_para_ticket = round(
            self.sub_total_calculado_para_ticket, 2
        )
        logger.debug(
            f"Subtotal calculado para este ticket (desborde): {self.sub_total_calculado_para_ticket}"
        )
