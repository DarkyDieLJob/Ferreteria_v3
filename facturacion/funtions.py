...
from django.http import (
    HttpResponse,
    JsonResponse,
)  # Added JsonResponse for potential error returns
...
def ciclo_desborde(fiscal=None):
    ...
    if not isinstance(fiscal, ComandoFiscal):
        logger.error(
            "Ciclo desborde: Se requiere una instancia válida de ComandoFiscal."
        )
        return  # Or raise error
    ...
    if num_tickets <= 0:
        logger.warning(
            "Ciclo desborde: cantidad_tickets es inválido o cero. No se generarán tickets."
        )
        return

    logger.info(
        f"Iniciando ciclo de comandos fiscales con desborde: {num_tickets} tickets a generar."
    )
    ...
    logger.info(
        f"Ciclo de comandos fiscales con desborde ({num_tickets} tickets) completado."
    )
...
def request_on_procesar_transaccion_to_dict(request):
    ...
    data = {
        ...
        "total_efectivo": request.POST.get(
            "total_efectivo"
        ),  # Might be missing if not cash
        "articulos_vendidos": request.POST.get(
            "articulos_vendidos"
        ),  # Usage of this field seems unclear later
...
def registrar_articulos_vendidos(request_dict):
    ...
    try:
        ...
        metodo_de_pago = MetodoPago.objects.get(
            id=request_dict.get("id_metodo_de_pago")
        )
        ...
        logger.debug(
            f"Objetos relacionados encontrados: Usuario='{usuario.username}', MetodoPago ID={metodo_de_pago.id}, Carrito ID={carrito.id}"
        )
        ...
        raise ValueError(
            f"Usuario '{request_dict.get('nombre_usuario')}' no encontrado."
        )
    except MetodoPago.DoesNotExist:
        logger.error(
            f"Método de pago con ID '{request_dict.get('id_metodo_de_pago')}' no encontrado."
        )
        raise ValueError(
            f"Método de pago ID '{request_dict.get('id_metodo_de_pago')}' inválido."
        )
    except Carrito.DoesNotExist:
        logger.error(
            f"Carrito con ID '{request_dict.get('carrito_id')}' no encontrado."
        )
        raise ValueError(f"Carrito ID '{request_dict.get('carrito_id')}' inválido.")
    except Exception as e:
        logger.error(
            f"Error inesperado al buscar objetos relacionados: {e}", exc_info=True
        )
        raise  # Re-raise unexpected errors

    # --- 2. Determine Amount Paid ---
    metodo_de_pago_display = (
        metodo_de_pago.display.lower() if metodo_de_pago.display else ""
    )
    total_request = request_dict.get("total", "0")
    total_efectivo_request = request_dict.get("total_efectivo")  # Can be None

    logger.debug(
        f"Determinando monto abonado: metodo='{metodo_de_pago_display}', total_req={total_request}, total_efectivo_req={total_efectivo_request}"
    )
    ...
    if (
        metodo_de_pago_display in ["efectivo con ticket", "efectivo s/ticket"]
        and total_efectivo_request is not None
    ):
    ...
    try:
        ...
        logger.error(
            f"Monto abonado inválido: '{monto_abonado_str}'. No se puede crear transacción."
        )
    ...
    if not items_en_carrito:
        error_msg = (
            f"Carrito ID {carrito.id} está vacío. No se puede procesar la transacción."
        )
    ...
    for idx, articulo in enumerate(items_en_carrito, 1):
        ...
        # Validate price
        precio = getattr(articulo, "precio", None) or getattr(
            getattr(articulo, "item", None), "precio", 0
        )
        if precio <= 0:
            descripcion = getattr(
                articulo, "descripcion", f"ID:{getattr(articulo, 'id', 'N/A')}"
            )
    ...
    try:
        ...
        for articulo_carrito in items_en_carrito:
            ...
            if isinstance(articulo_carrito, Articulo):
                ...
                logger.debug(
                    f"Creando ArticuloVendido para Item ID {articulo_carrito.item.id} (Cant: {articulo_carrito.cantidad})"
                )
        ...
        if articulos_vendidos_creados:
            ...
            logger.info(
                f"{len(articulos_vendidos_creados)} ArticuloVendido asociados a la transacción ID {transaccion.id}."
            )
        else:
            logger.info(
                f"No se asociaron ArticuloVendido a la transacción ID {transaccion.id} (carrito vacío)."
            )
    ...
    json_para_websocket = (
        []
    )  # Initialize as empty list (as TicketFactura returns a list)
    ...
    if (
        id_metodo_pago_int != 1 and id_cliente_str is not None
    ):  # Need client info (even if empty for Consumidor Final)
        ...
        if not id_cliente_str:  # Handle empty string as Consumidor Final default (ID 1)
            ...
            logger.debug(
                "ID Cliente vacío, usando ID 1 (Consumidor Final) para ticket."
            )
        else:
            try:
                ...
                logger.error(
                    f"ID de cliente inválido ('{id_cliente_str}') para generar ticket. Abortando generación de JSON."
                )
        ...
        if cliente_id_para_ticket is not None:
            try:
                ...
                logger.info(
                    f"Cliente ID {cliente_id_para_ticket} asignado a la transacción ID {transaccion.id}."
                )
                ...
                logger.error(
                    f"Cliente con ID {cliente_id_para_ticket} no encontrado al intentar asignar a transacción. No se generará JSON."
                )
        ...
        logger.info(
            "No se requiere ticket fiscal (Método de pago ID 1 o ID Cliente no proporcionado)."
        )
