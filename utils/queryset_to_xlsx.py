...
def queryset_to_xlsx(queryset, filename):
    ...
    if isinstance(queryset.first(), dict):
        ...
        for col_num, header in enumerate(headers, 1):
            ...
            sheet.column_dimensions[openpyxl.utils.get_column_letter(col_num)].width = (
                len(header)
            )
            sheet.cell(row=1, column=col_num).alignment = openpyxl.styles.Alignment(
                horizontal="center"
            )
        ...
        for row_num, row in enumerate(queryset, 2):
            for col_num, (key, value) in enumerate(row.items(), 1):
                ...
                sheet.cell(row=row_num, column=col_num).alignment = (
        ...
        for row_num, row in enumerate(queryset, 2):
            for col_num, field in enumerate(headers, 1):
                value = getattr(
                    row, field.name
                )  # Use getattr() to access the field value
                if isinstance(value, datetime):
                    value = value.replace(
                        tzinfo=None
                    )  # Eliminar la información de zona horaria
...
def diccionario_to_xlsx(diccionario, filename):
    ...
    for col_num, header in enumerate(headers, 1):
        sheet.cell(row=1, column=col_num).value = header
        sheet.column_dimensions[openpyxl.utils.get_column_letter(col_num)].width = len(
            header
        )
        sheet.cell(row=1, column=col_num).alignment = openpyxl.styles.Alignment(
            horizontal="center"
    ...
    for row_num, key in enumerate(diccionario, 2):
        ...
        sheet.column_dimensions["A"].width = max(
            sheet.column_dimensions["A"].width, len(str(key))
        )
        sheet.column_dimensions["B"].width = max(
            sheet.column_dimensions["B"].width, len(str(diccionario[key]))
        )
        sheet.cell(row=row_num, column=1).alignment = openpyxl.styles.Alignment(
            horizontal="center"
        )
        sheet.cell(row=row_num, column=2).alignment = openpyxl.styles.Alignment(
            horizontal="center"
        )
...
def ejecutar():
    ...
    articulos_vendidos_contados = (
        ArticuloVendido.objects.annotate(month=TruncMonth("transaccion__fecha"))
        .values("month", "item__codigo", "item__descripcion")
        .annotate(count=Count("item"), total_quantity=Sum("cantidad"))
