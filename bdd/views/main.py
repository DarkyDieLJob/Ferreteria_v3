...
class BusquedaView(FormView):
    ...
    def form_valid(self, form):
        ...
        logger.debug(
            f"Criterios de búsqueda: Marca={marca}, Cajon={cajon}, Cajonera={cajonera}, Sector={sector}"
        )
        ...
        context = self.get_context_data(
            form=form
        )  # Obtiene contexto base (incluye sectores)
...
class ItemsView(View):
    def get(self, request, cajon_id):
        ...
        try:
            ...
            logger.warning(
                f"Intento de acceder a items de cajón inexistente (ID: {cajon_id})."
            )
            return JsonResponse({"error": "Cajón no encontrado"}, status=404)
        except Exception as e:
            logger.error(
                f"Error en ItemsView para cajon_id={cajon_id}: {e}", exc_info=True
            )
...
class Imprimir(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.ruta_actual == "/imprimir/":
            ...
            context["form"] = MyForm(
                model_name="Item", fields_to_show=lista_campos
            )  # Asume modelo Item
            ...
            context["datos"] = kwargs.get(
                "datos", []
            )  # Obtener datos pasados desde POST/GET

        else:
            logger.warning(
                f"Ruta no manejada explícitamente en Imprimir.get_context_data: {self.ruta_actual}"
            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.path == "/imprimir/tabla/":
            logger.warning(
                "Acceso GET a /imprimir/tabla/, mostrando tabla vacía o según params GET."
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            try:
                ...
                logger.debug(
                    f"Datos preparados para la plantilla de tabla: {len(datos_filtrados)} filas."
                )

            except Exception as e:
                logger.error(
                    f"Error al filtrar Items para imprimir con datos {form_data}: {e}",
                    exc_info=True,
                )
...
class ListadoPedidos(MiVista):
    def get(self, request, *args, **kwargs):
        ...
        if (
            model_name and model_name == "Lista_Pedidos"
        ):  # Asegurarse que es el modelo correcto
            ...
            for k, v in query_params.items():
                ...
                # Manejo de FK: si el campo es una FK y el modelo relacionado tiene 'descripcion'
                elif field_name in foreign_keys and hasattr(
                    foreign_keys[field_name], "descripcion"
                ):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Parámetro '{field_name}' ignorado en ListadoPedidos (no válido/manejado)."
                        )
            ...
            if filter_kwargs:
                try:
                    ...
                    context["titulos"] = (
                        campos_a_mostrar  # Sobreescribir títulos si es necesario
                    )
                except Exception as e:
                    logger.error(
                        f"Error buscando en Lista_Pedidos con filtros {filter_kwargs}: {e}",
                        exc_info=True,
                    )
                    context["search_error"] = (
                        "Error al realizar la búsqueda de pedidos."
                    )
                    datos_filtrados = []
            else:
            ...
            logger.error(
                f"ListadoPedidos esperaba modelo 'Lista_Pedidos', pero el Armador indicó '{model_name}'."
            )
...
class ListarCarteles(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["campos_tabla"] = [
            "descripcion"
        ]  # Campos a mostrar en tabla? Solo descripción?
        ...
        )  # Añadir datos pasados desde GET
    ...
    def get(self, request, *args, **kwargs):
        ...
        logger.debug(
            f"Filtros GET: proveedor_id={proveedor_id}, revisar={filtro_revisar} (param: {revisar_param})"
        )
        ...
        if proveedor_id:
            ...
            if filtro_revisar:
                ...
                cajones_qs = cajones_qs.filter(
                    Exists(
                        Item.objects.filter(
                            cajon=OuterRef("id"),
                            proveedor_id=proveedor_id,
                            carteles__revisar=True,
                        )
                    )
                )
                logger.debug(
                    f"Filtrando cajones por proveedor_id={proveedor_id} Y que tengan items de ese proveedor a revisar."
                )
            else:
                ...
                cajones_qs = cajones_qs.filter(
                    Exists(
                        Item.objects.filter(
                            cajon=OuterRef("id"), proveedor_id=proveedor_id
                        )
                    )
                )
                logger.debug(
                    f"Filtrando cajones por proveedor_id={proveedor_id} (sin filtro 'revisar' obligatorio)."
                )
        ...
        for cajon in cajones_qs:
            ...
            items_del_cajon = [
                item
                for item in cajon.item_set.all()  # Usar prefetch
                if item.tiene_cartel
                and (
                    not filtro_revisar
                    or item.carteles_set.filter(revisar=True).exists()
                )
            ]
            if (
                items_del_cajon
            ):  # Solo añadir cajón si tiene items relevantes después de filtrar
                for item in items_del_cajon:
                    item.url = f"/x_cartel/imprimir/{item.id}/"  # URL para imprimir
                    item.needs_review = item.carteles_set.filter(
                        revisar=True
                    ).exists()  # Flag para la plantilla
                cajon.items_filtrados = items_del_cajon  # Guardar items filtrados
                cajones_list.append(cajon)
                logger.debug(
                    f"Cajon '{cajon.codigo}' añadido con {len(items_del_cajon)} items filtrados."
                )
        ...
        extra_context = {
            ...
            "busqueda_realizada": bool(
                proveedor_id or revisar_param != "false"
            ),  # Indica si se aplicó filtro
        }
    ...
    def post(self, request, *args, **kwargs):
        ...
        logger.warning(
            "Recibida solicitud POST en ListarCarteles, pero no implementada. Redirigiendo a GET?"
        )
