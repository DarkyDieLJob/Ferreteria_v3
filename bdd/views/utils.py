...
def articulo_to_dict(articulo):
    ...
    logger.debug(
        f"Convirtiendo a dict: {type(articulo)} - ID: {getattr(articulo, 'id', 'N/A')}"
    )
    if isinstance(articulo, Articulo):
        ...
        return {
            ...
            "descripcion": (
                getattr(articulo.item, "descripcion", "N/A") if articulo.item else "N/A"
            ),  # Añadir descripción
            "cantidad": articulo.cantidad,
            "precio": str(articulo.precio),  # Convertir Decimal a string para JSON
            "precio_efectivo": str(
                articulo.precio_efectivo
        ...
        logger.warning(
            f"articulo_to_dict recibió un tipo no esperado: {type(articulo)}"
        )
        return {}
...
def calcular_total(datos):
    ...
    for nombre_carrito, carrito_data in datos.items():
        if not isinstance(carrito_data, dict):
            logger.warning(
                f"Elemento para '{nombre_carrito}' no es un diccionario, saltando cálculo de total."
            )
            continue
        ...
        if not isinstance(articulos, list):
            logger.warning(
                f"'articulos' para '{nombre_carrito}' no es una lista, saltando."
            )
            articulos = []
        ...
        for articulo in articulos:
            try:
                ...
                if cantidad < 0:
                    logger.warning(
                        f"Cantidad negativa encontrada en {nombre_carrito} - Articulo ID {articulo.get('id')}. Ignorando para total."
                    )
                    continue
                ...
                logger.error(
                    f"Error al procesar artículo {articulo.get('id')} en carrito '{nombre_carrito}': {e}. Artículo: {articulo}"
                )
            except Exception as e:
                logger.error(
                    f"Error inesperado procesando artículo {articulo.get('id')} en carrito '{nombre_carrito}': {e}",
                    exc_info=True,
                )
        ...
        if not isinstance(articulos_sr, list):
            logger.warning(
                f"'articulos_sin_registro' para '{nombre_carrito}' no es una lista, saltando."
            )
            articulos_sr = []
        ...
        for articulo_sr in articulos_sr:
            try:
                ...
                if cantidad < 0:
                    logger.warning(
                        f"Cantidad negativa encontrada en {nombre_carrito} - ArticuloSinRegistro ID {articulo_sr.get('id')}. Ignorando."
                    )
                    continue
                ...
                logger.error(
                    f"Error al procesar art sin registro {articulo_sr.get('id')} en carrito '{nombre_carrito}': {e}. Artículo: {articulo_sr}"
                )
            except Exception as e:
                logger.error(
                    f"Error inesperado procesando art sin registro {articulo_sr.get('id')} en carrito '{nombre_carrito}': {e}",
                    exc_info=True,
                )
...
def carrito_to_dict(carrito):
    ...
    return {
        "id": carrito.id,
        "usuario": (
            carrito.usuario.username if carrito.usuario else "Usuario no asignado"
        ),
