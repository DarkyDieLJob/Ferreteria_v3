...
from ..models import (
    ...
    Listado_Planillas,
    Lista_Pedidos,
)  # Importa desde el models.py de la app padre
...
class MiVista(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            if armador:
                ...
                logger.debug(
                    f"Armador settings: metodo={context['metodo']}, muro={context['muro']}, modelo={model_name}"
                )
                ...
                if model:
                    modelo_campos = list(
                        armador.modelo_campos.values_list("nombre", flat=True)
                    )
                    if modelo_campos and modelo_campos[0] == "__all__":
                        titulos = [
                            field.name
                            for field in model._meta.get_fields()
                            if field.name in [f.name for f in model._meta.fields]
                            and not (
                        ...
                        titulos = [
                            field.name
                            for field in model._meta.get_fields()
                            if field.name in modelo_campos
                            and field.name in [f.name for f in model._meta.fields]
                            and not (
                                isinstance(field, ForeignKey)
                                and field.related_model != model
                            )
                        ...
                        logger.warning(
                            f"Armador {armador.nombre} no tiene modelo_campos definidos."
                        )
                ...
                campos_contenedor = [
                    ...
                    "b",
                    "c",
                ]  # Asumiendo que estos son los campos relevantes en Contenedor
                for campo in campos_contenedor:
                    try:
                        ...
                        try:
                            ...
                            logger.debug(
                                f"Template '{template_name}' añadido desde contenedor."
                            )
                        except (
                            Exception
                        ):  # TemplateDoesNotExist más específico sería mejor
                            logger.warning(
                                f"Template '{template_name}' definido en Contenedor no encontrado."
                            )
                    except AttributeError:
                        logger.warning(
                            f"Contenedor asociado al Armador no tiene el campo '{campo}'."
                        )
                    except Exception as e:
                        logger.error(
                            f"Error procesando campo '{campo}' del contenedor: {e}",
                            exc_info=True,
                        )
                ...
                logger.warning(
                    f"No se encontró Armador para la ruta: {self.ruta_actual}. Algunas partes del contexto pueden faltar."
                )
            ...
            logger.error(
                f"Error crítico obteniendo datos del Armador para ruta {self.ruta_actual}: {e}",
                exc_info=True,
            )
        ...
        try:
            context["seleccion_descargar"] = Listado_Planillas.objects.filter(
                descargar=True
            ).order_by(
                "-id"
            )  # Reemplaza [::-1]
            ...
            logger.error(
                f"Error al obtener datos de Listado_Planillas: {e}", exc_info=True
            )
        ...
        if getattr(settings, "INTEGRATE_MERCADOPAGO", False) and getattr(
            settings, "INTERNET", False
        ):  # Usa settings
            try:
                mp_token_file = getattr(
                    settings, "MP_TOKEN_FILE", "./mp_access_token.txt"
                )
                ...
                def buscar_pagos_mp(begin_date, end_date):
                    ...
                    if result["status"] != 200 or "results" not in result["response"]:
                        logger.error(
                            f"Error en la respuesta de búsqueda de pagos MP: Status={result['status']}, Response={result['response']}"
                        )
                        return []
                    ...
                    for py in payments:
                        try:
                            ...
                            identificador = py.get("payer", {}).get("id") or py.get(
                                "payer_id"
                            )  # Prioriza payer.id
                            date = datetime.strptime(
                                date_string.split(".")[0], "%Y-%m-%dT%H:%M:%S"
                            )  # Ignora microsegundos y TZ para simplificar
                            hour = date.strftime("%H:%M:%S")
                            total_paid_amount = py.get("transaction_details", {}).get(
                                "total_paid_amount", 0
                            )
                            ...
                            logger.warning(
                                f"Error procesando pago individual de MP (ID: {py.get('id')}): {e}. Pago: {py}"
                            )
                ...
                logger.error(
                    f"Archivo de token de MercadoPago no encontrado en: {mp_token_file}"
                )
                context["payments"] = []
                context["tabla_mp"] = []
            except Exception as e:
                logger.error(
                    f"Error inesperado con la integración de MercadoPago: {e}",
                    exc_info=True,
                )
                context["payments"] = []
                context["tabla_mp"] = []
    ...
    def post(self, request, *args, **kwargs):
        ...
        lista_campos = context.get(
            "lista_formulario_campos", ["None"]
        )  # Default seguro

        if not model_name:
            logger.error(
                "Intento de POST sin model_name definido en el contexto (probablemente Armador no encontrado)."
            )
            # Decide cómo manejar esto: mostrar error, redirigir, etc.
            context["form_errors"] = (
                "Error de configuración: No se pudo determinar el modelo para guardar."
            )
        ...
        if form.is_valid():
            ...
            if saved_object:
                logger.info(
                    f"Objeto {model_name} (ID: {saved_object.pk}) guardado/actualizado exitosamente. Redirigiendo."
                )
                return redirect(
                    self.request.path
                ...
                context["form_errors"] = (
                    "Ocurrió un error al guardar los datos."  # Mensaje genérico para el usuario
                )
                context["form"] = (
                    form  # Mantener el formulario con los datos ingresados
                )
        else:
            logger.warning(
                f"Formulario POST inválido para modelo '{model_name}'. Errores: {form.errors}"
            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        titulos = context.get(
            "titulos", []
        )  # Obtener los títulos definidos en get_context_data

        if not model_name:
            logger.warning(
                "Intento de GET (búsqueda) sin model_name definido (Armador no encontrado?)."
            )
        ...
        if armador and armador.busqueda:
            ...
            query_params = {
                ...
                if v and k not in ["page", "csrfmiddlewaretoken"]
            ...
            if model:
                campos_contiene = (
                    [obj.nombre for obj in armador.formulario_campos_contiene.all()]
                    if armador
                    else []
                )
                campos_empieza = (
                    [obj.nombre for obj in armador.formulario_campos_empieza_con.all()]
                    if armador
                    else []
                )
                ...
                for k, v in query_params.items():
                    ...
                    if field_name in campos_contiene:
                        lookup_type = (
                            "__icontains"  # Usar icontains para case-insensitive
                        )
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Parámetro de búsqueda '{k}' ignorado porque no es un campo válido del modelo '{model_name}' ni un campo especial."
                            )
                ...
                if filter_kwargs:
                    try:
                        ...
                        if model_name == "Item":
                            logger.debug(
                                "Anotando queryset de Item con 'tiene_pedido'."
                            )
                        ...
                        logger.error(
                            f"Error al ejecutar la búsqueda en modelo '{model_name}' con filtros {filter_kwargs}: {e}",
                            exc_info=True,
                        )
                        context["datos"] = []
                ...
                logger.error(
                    "No se pudo obtener el modelo para realizar la búsqueda GET."
                )
                context["datos"] = []
        else:
            ...
            logger.debug(
                "Procesando solicitud GET estándar (no búsqueda o formulario inicial)."
            )
