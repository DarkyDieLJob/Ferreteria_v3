...
# Importar las vistas
from core_testing.views import (
...
class CoreTestingURLTests(TestCase):
    ...
    def test_run_detail_url_resolves(self):
        ...
        url = reverse('core_testing:run_detail', kwargs={'run_id': self.test_run.id})
        self.assertEqual(url, f'/testing/runs/{self.test_run.id}/')
        
        # Verificar que la vista asociada sea la correcta
        resolver = resolve(f'/testing/runs/{self.test_run.id}/')
        self.assertEqual(resolver.func.__name__, TestRunDetailView.as_view().__name__)
    ...
    def test_nonexistent_run_detail_returns_404(self):
        ...
        url = reverse('core_testing:run_detail', kwargs={'run_id': non_existent_id})
        response = self.client.get(url)
