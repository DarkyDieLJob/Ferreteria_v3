...
def buscar_modificar_registros(csv_file, filtro):
    ...
    with open(csv_file, newline="") as csvfile:
        ...
        for row in reader:
            ...
            if not (
                row["precio_base"] is None
                or row["precio_base"] == ""
                or row["precio_base"] == " "
                or row["precio_base"] == "-"
                or row["precio_base"] == "."
                or row["precio_base"] == "#N/A"
                or row["precio_base"] == "#VALUE!"
            ):
...
def fusionar_hojas(file):
    ...
    # Fusionar todas las hojas en una sola
    df = pd.concat(
        [pd.DataFrame(ws.values) for ws in wb.worksheets if ws.title != "general"]
    )
...
def principal():
    ...
    for i in items:
        try:
            ...
            print(
                "No se pudo get_or_create [{}] , [{}]".format(i["name"], i["id"]),
                flush=True,
            )

    try:
        # Obtener el archivo de Google Drive
        planilla = Listado_Planillas.objects.filter(proveedor__text_display="Caramia")[
            0
        ]
        file_id = planilla.identificador
        request = drive_service.files().get_media(fileId=file_id)
    ...
    for sp in lista:
        ...
        if id_archivo_proveedor:
            request = patoba.drive_service.files().get_media(
                fileId=id_archivo_proveedor
            )
            ...
            hoja_proveedor = pd.read_excel(hoja_proveedor, sheet_name=None)
        else:
            print(
        ...
        identificador = patoba.obtener_id_por_nombre(nombre_plantilla, cosnt.PLANTILLAS)
