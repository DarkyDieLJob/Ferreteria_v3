...
def descargar_archivo_drive(
    ...
    while not done:
        ...
        if status:
            logger.debug("Descarga %s: %s%%", file_id, int(status.progress() * 100))
    ...
    try:
        ...
        if sub_carpeta_nombre:
            try:
                ...
                logger.error(
                    f"Error al get_or_create Sub_Carpeta '{sub_carpeta_nombre}' para código {codigo_item}: {sc_e}"
                )
        ...
        if sub_titulo_nombre:
            try:
                ...
                logger.error(
                    f"Error al get_or_create Sub_Titulo '{sub_titulo_nombre}' para código {codigo_item}: {st_e}"
                )
        ...
        logger.error(
            f"Error CRÍTICO al procesar/guardar registro para código '{codigo_item}'. Fila original: {row_original}"
        )
...
def buscar_modificar_registros(csv_file, filtro):
    ...
    try:
        ...
        with open(
        ...
        ) as csvfile:  # Especificar encoding
            ...
            for row in reader:
                ...
                if "sub_carpeta" not in row:
                    row["sub_carpeta"] = None
                if "sub_titulo" not in row:
                    row["sub_titulo"] = None

                crear_o_actualizar_registro(
                    row
                )  # Llama a la función refactorizada con logging
                processed_count += 1

        logger.info(
            f"Carga INDIVIDUAL desde CSV '{csv_file}' completada para filtro '{filtro}'. Procesados: {processed_count}, Omitidos: {skipped_count}"
        )
        ...
        logger.error(
            f"Error al leer o procesar el archivo CSV '{csv_file}' en buscar_modificar_registros."
        )
        logger.exception(e)
...
def fusionar_hojas(wb):  # Modificado para recibir Workbook, no file path/object
    ...
    for ws in wb.worksheets:
        if ws.title.lower() != "general":  # Comparación insensible a mayúsculas
            ...
            if sheet_data:  # Solo añadir si la hoja tiene datos
                ...
                logger.debug(
                    f"Hoja '{ws.title}' está vacía o no se pudieron leer datos."
                )
    ...
    logger.info(
        f"Datos de {sheet_count} hojas recopilados. Creando hoja 'cargar datos'."
    )
    ...
    try:
        ...
        logger.warning(
            "dataframe_to_rows no disponible. Usando método alternativo (puede ser más lento)."
        )
...
def marcar_revisar_carteles(id_proveedor):
    ...
    try:
        ...
        count_cc = CartelesCajon.objects.filter(proveedor=id_proveedor).update(
            revisar=True
        )
        logger.info(f"{count_cc} CartelesCajon marcados.")
    except Exception as e:
        logger.error(
            f"Error al marcar carteles para revisar para proveedor ID {id_proveedor}."
        )
        logger.exception(e)
...
def principal():
    ...
    try:
        ...
        if not gmail_service:
            logger.warning(
                "Servicio de Gmail no inicializado. get_emails no funcionará."
            )
    ...
    try:
        if gmail_service and drive_service:
            ...
            )  # Asume que esta función tiene su propio logging interno
    ...
    try:
        ...
        for item_info in items_drive:
            ...
            if not item_name or not item_id:
                logger.warning(
                    f"Item de Drive omitido por falta de 'name' o 'id': {item_info}"
                )
                skipped_count += 1
                continue
            try:
                ...
                if created:
                    logger.info(
                        f"Listado_Planillas CREADO: ID={item_id}, Desc={item_name}"
                    )
                    created_count += 1
                ...
                logger.error(
                    f"Error en update_or_create Listado_Planillas para ID='{item_id}', Nombre='{item_name}': {db_e}"
                )
                skipped_count += 1
        logger.info(
            f"Listado de Drive procesado. Verificados: {checked_count}, Creados: {created_count}, Omitidos/Errores: {skipped_count}"
        )

    except HttpError as http_e:
        logger.error(
            f"Error HTTP al listar archivos de Drive Inbox (ID: {const.INBOX}): {http_e}"
        )
    except Exception as e:
        logger.error(
            "Error inesperado al listar archivos de Drive o actualizar Listado_Planillas."
        )
        logger.exception(e)

    logger.info("Bloque 'Caramia' desactivado: proveedor sin soporte en esta versión.")

    # --- 3. Obtener Nombres de Hojas para Planillas Pendientes ---
    logger.info(
        "Obteniendo nombres de hojas para planillas pendientes (listo=False, descargar=False)..."
    )
    ...
    for dato in datos_pendientes:
        logger.debug(
            f"Procesando hojas para planilla ID={dato.identificador}, Desc='{dato.descripcion}'"
        )
        try:
            ...
            if http_e.resp.status == 404:
                logger.warning(
                    f"Archivo no encontrado (404) al obtener hojas para ID={dato.identificador}. Eliminando registro."
                )
                try:
                    dato.delete()
                    ...
                    logger.error(
                        f"Error al eliminar registro ID={dato.identificador} después de 404: {del_e}"
                    )
                    errors_sheets += 1
            else:
                logger.error(
                    f"Error HTTP ({http_e.resp.status}) al obtener hojas para ID={dato.identificador}: {http_e}"
                )
                errors_sheets += 1
        except Exception as e:
            logger.error(
                f"Error (no HTTP) al leer hojas (pandas?) para ID={dato.identificador}"
            )
            logger.exception(e)
            errors_sheets += 1
    logger.info(
