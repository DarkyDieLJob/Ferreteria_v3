...
class ActualizadorConfig(AppConfig):
    ...
    def ready(self):
        ...
        """
        if os.environ.get("RUNNING_ACTUALIZADOR_SCRIPT") == "1":
            logger.debug(
        ...
        with _lock_inicializacion:
            if not _worker_inicializado:
                ...
                logger.info(
                    f"App '{self.name}' lista (en hilo {thread_id}). Intentando iniciar ColaTareasWorker..."
                )
                ...
                try:
                    ...
                    _worker_inicializado = (
                        True  # Marcar como inicializado en este proceso
                    )

                except ImportError:
                    logger.exception(
                        f"Error Crítico: No se pudo importar 'ColaTareasWorker' desde '.tasks'. ¡El worker de tareas no se iniciará!"
                    )
                except Exception as e:
                    # Captura cualquier otro error durante la inicialización del worker
                    logger.exception(
                        f"Error Crítico al intentar iniciar ColaTareasWorker en ready(): {e}"
                    )
            else:
                logger.debug(
                    f"App '{self.name}' lista: El worker ya fue inicializado en este proceso."
                )
