...
def custom_round(price):
    ...
    try:
        ...
        logger.warning(
            f"custom_round (compat) falló para valor: '{price}'. Devolviendo 0."
        )
        return 0
...
def get_batch_size(default=1000):
    ...
    try:
        ...
        env_val = os.getenv("ACT_CSV_BATCH_SIZE")
...
def get_efectivo_discount_pct(default=0.0):
    ...
    try:
        if (
            ...
            and const.ACT_CSV_EFECTIVO_DESCUENTO_PCT is not None
        ):
            return float(const.ACT_CSV_EFECTIVO_DESCUENTO_PCT)
        env_val = os.getenv("ACT_CSV_EFECTIVO_DESCUENTO_PCT")
...
def crear_o_actualizar_registro(row_original):
    ...
    try:
        ...
        if sub_carpeta_nombre:
            sub_carpeta, created_sc = Sub_Carpeta.objects.get_or_create(
                nombre=sub_carpeta_nombre
            )
            if created_sc:
        ...
        if sub_titulo_nombre:
            sub_titulo, created_st = Sub_Titulo.objects.get_or_create(
                nombre=sub_titulo_nombre
            )
            if created_st:
        ...
        if not isinstance(final_efectivo, (int, float)) or final_efectivo <= 0:
            if isinstance(final, (int, float)):
                ...
                logger.debug(
                    f"Ajustado final_efectivo para código {codigo_item} al valor de final: {final}"
                )
            else:
                row["final_efectivo"] = (
                    0  # Valor por defecto si 'final' tampoco es válido
                )
                logger.warning(
                    f"final_efectivo y final inválidos para código {codigo_item}. Establecido a 0."
                )
        ...
        if (
            ...
            and not isinstance(defaults["sub_carpeta"], Sub_Carpeta)
            and defaults["sub_carpeta"] is not None
        ):
            logger.error(
                f"Valor inválido para sub_carpeta en defaults para código {codigo_item}. Omitiendo."
            )
            defaults.pop("sub_carpeta")  # O manejar de otra forma
        if (
            "sub_titulo" in defaults
            and not isinstance(defaults["sub_titulo"], Sub_Titulo)
            and defaults["sub_titulo"] is not None
        ):
            logger.error(
                f"Valor inválido para sub_titulo en defaults para código {codigo_item}. Omitiendo."
            )
        ...
        logger.error(
            f"Error al procesar/guardar registro para código '{codigo_item}'. Fila original: {row_original}"
        )
        ...
        try:
            ...
            logger.info(
                f"Fila con error para código '{codigo_item}' guardada en {error_file}"
            )
        except Exception as csv_e:
            logger.error(
                f"¡FALLO AL ESCRIBIR EN CSV DE ERRORES! Error: {csv_e}. Fila: {row_original}"
            )
...
def crear_o_actualizar_registros_en_lotes(rows, tamaño_lote=1000):
    ...
    codigos_referenciados = [row.get("codigo") for row in rows if row.get("codigo")]
    # 'codigo' no es único: construir un mapeo manual tomando la primera coincidencia encontrada
    items_existentes = {}
    if codigos_referenciados:
        for item in Item.objects.filter(codigo__in=codigos_referenciados).only(
            "id", "codigo"
        ):
            if item.codigo not in items_existentes:
                items_existentes[item.codigo] = item
    ...
    for row_original in rows:
        ...
        if not codigo_item:
            ...
            codigos_con_error.add(
                str(row_original)
            )  # Guardar representación de la fila
            continue
        ...
        if any(
            ...
            if key not in ["sub_carpeta", "sub_titulo", "codigo"]
        ):
            logger.warning(
                f"Fila omitida para código '{codigo_item}' por contener valores vacíos/nulos: {row_original}"
            )
        ...
        try:
            ...
            if sub_carpeta_nombre:
                if sub_carpeta_nombre in subcarpetas_cache:
                    ...
                    sub_carpeta, created = Sub_Carpeta.objects.get_or_create(
                        nombre=sub_carpeta_nombre
                    )
                    if created:
                        logger.info(f"Sub_Carpeta creada: '{sub_carpeta_nombre}'")
                    subcarpetas_cache[sub_carpeta_nombre] = (
                        sub_carpeta  # Actualizar cache
                    )
            ...
            if sub_titulo_nombre:
                if sub_titulo_nombre in subtitulos_cache:
                    ...
                    sub_titulo, created = Sub_Titulo.objects.get_or_create(
                        nombre=sub_titulo_nombre
                    )
                    if created:
                        logger.info(f"Sub_Titulo creado: '{sub_titulo_nombre}'")
            ...
            try:  # Convertir a float antes de comparar
                final_efectivo_f = (
                    float(str(final_efectivo).replace(",", "."))
                    if final_efectivo
                    else 0
                )
                ...
                logger.warning(
                    f"Valor no numérico en final/final_efectivo para código {codigo_item}. Usando 0."
                )
                final_efectivo_f = 0
            ...
            item_data = {
                ...
                if v is not None or k in ["sub_carpeta", "sub_titulo"]
            }  # Mantener FKs aunque sean None

            if codigo_item in items_existentes:
                # Para actualizar: reutilizar la instancia existente para conservar la PK
                item_obj = items_existentes[codigo_item]
                for campo, valor in item_data.items():
                    if campo == "codigo":
                        continue
                    setattr(item_obj, campo, valor)
            ...
            logger.error(
                f"Error al preparar datos para código '{codigo_item}'. Fila original: {row_original}"
            )
            logger.exception(e)
    ...
    fin_prep = time.time()
    logger.info(
        f"Fase 1 completada en {fin_prep - inicio_prep:.2f} seg. Items a crear: {len(items_para_crear)}, a actualizar: {len(mapa_items_para_actualizar)}, con error: {len(codigos_con_error)}"
    )
    ...
    if codigos_con_error:
        logger.warning(
            f"Se encontraron errores al preparar/procesar {len(codigos_con_error)} filas/códigos."
        )
...
def desactualizar_anteriores(filtro):
    ...
    try:
        ...
        logger.info(
            f"{count} items marcados como no actualizados para filtro '{filtro}'."
        )
    except Exception as e:
...
def buscar_modificar_registros(csv_file, filtro):
        ...
        with open(csv_file, newline="", encoding="utf8") as csvfile:
            ...
            for row in reader:
                ...
                if precio_base in [
                    ...
                    "#N/A",
                    "#VALUE!",
                ] or not validar_digitos_str(str(precio_base)):
                    logger.debug(
                        f"Fila omitida por precio_base inválido ('{precio_base}') para código {row.get('codigo', 'N/A')}"
                    )
                    continue

                # Aplicar redondeo unificado (sin cartel en esta etapa) si los precios son válidos
                try:
                    if "final" in row and validar_digitos_str(str(row["final"])):
                        row["final"] = round_price(row["final"], is_cartel=False)
                    if "final_efectivo" in row and validar_digitos_str(
                        str(row["final_efectivo"])
                    ):
                        row["final_efectivo"] = round_price(
                            row["final_efectivo"], is_cartel=False
                        )
                except Exception as round_e:
                    logger.warning(
                        f"Error aplicando redondeo custom a fila {row.get('codigo', 'N/A')}: {round_e}"
                    )

                crear_o_actualizar_registro(
                    row
                )  # Llama a la función que ya tiene logging
                processed_count += 1
    ...
    if error_count == 0:
        logger.info(
            f"Procesamiento individual de '{csv_file}' (filtro '{filtro}') completado. {processed_count} filas procesadas."
        )
    else:
        logger.error(
            f"Procesamiento individual de '{csv_file}' (filtro '{filtro}') finalizado con errores."
        )
...
def buscar_modificar_registros_lotes(csv_file, filtro):
    ...
    logger.info(
        f"Iniciando procesamiento por lotes desde CSV: '{csv_file}' para filtro: '{filtro}'. Lote={batch_size}, Descuento efectivo={descuento_pct}%"
    )
    desactualizar_anteriores(filtro)
    rows = []
    read_count = 0
    ...
    try:
        with open(csv_file, newline="", encoding="utf8") as csvfile:
            ...
            for row in reader:
                ...
                # Aplicar descuento sobre 'final_efectivo' si corresponde (antes de redondear)
                try:
                    if (
                        descuento_pct
                        and "final_efectivo" in row
                        and validar_digitos_str(str(row["final_efectivo"]))
                    ):
                        fe_val = float(str(row["final_efectivo"]).replace(",", "."))
                        factor = max(0.0, 1.0 - (descuento_pct / 100.0))
                        row["final_efectivo"] = fe_val * factor
                except Exception as dsc_e:
                    logger.warning(
                        f"Error aplicando descuento efectivo a fila {row.get('codigo', 'N/A')}: {dsc_e}"
                    )
                ...
                try:
                    ...
                    logger.warning(
                        f"Error aplicando redondeo custom a fila {row.get('codigo', 'N/A')} durante carga de lotes: {round_e}"
                    )
        ...
        logger.info(
            f"Archivo CSV '{csv_file}' leído. Filas leídas: {read_count}, omitidas por precio_base inválido: {skipped_count}. Filas a procesar: {len(rows)}"
        )
        if rows:
            ...
            logger.info(
                f"No hay filas válidas para procesar en lotes desde '{csv_file}'."
            )
    ...
    logger.info(
        f"Procesamiento por lotes de '{csv_file}' (filtro '{filtro}') completado."
    )
...
def principal_csv():
    ...
    try:
        ...
        logger.info(
            f"Proveedores encontrados con CSV pendiente: {[p.nombre for p in proveedores]}"
        )
        ...
        for proveedor in proveedores:
            ...
            try:
                ...
                if not id_archivo_plantilla:
                    logger.error(
                        f"No se encontró ID de plantilla en Drive para '{proveedor.nombre}' en carpeta PLANTILLAS (ID: {const.PLANTILLAS}). Saltando descarga."
                    )
                    continue  # Saltar al siguiente proveedor

                logger.info(
                    f"Obteniendo datos de Google Sheet '{proveedor.nombre}' (ID: {id_archivo_plantilla}), Rango: 'BDD'"
                )
                result = (
                ...
                if not values:
                    logger.warning(
                        f"No se encontraron datos en la hoja 'BDD' de la plantilla '{proveedor.nombre}'. Creando CSV vacío."
                    )
                    ...
                    logger.info(
                        f"Datos guardados en '{ruta_csv}' ({len(values)} filas)."
                    )

            except Exception as e:
                logger.error(
                    f"Error al descargar/guardar datos de Google Sheet para '{proveedor.nombre}' (ID: {id_archivo_plantilla})."
                )
                logger.exception(e)
            ...
            try:
                ...
                logger.error(
                    f"Fallo general al llamar a buscar_modificar_registros_lotes para '{ruta_csv}'."
                )
            ...
            try:
                ...
                prov_instance.save()
                logger.info(
                    f"Proveedor '{proveedor.nombre}' marcado como procesado (hay_csv_pendiente=False)."
                )

                carteles_updated = Carteles.objects.filter(
                    proveedor=proveedor.id
                ).update(revisar=True)
                logger.info(
                    f"{carteles_updated} Carteles marcados para revisar para proveedor '{proveedor.nombre}'."
                )
                carteles_cajon_updated = CartelesCajon.objects.filter(
                    proveedor=proveedor.id
                ).update(revisar=True)
                logger.info(
                    f"{carteles_cajon_updated} CartelesCajon marcados para revisar para proveedor '{proveedor.nombre}'."
                )

            except ListaProveedores.DoesNotExist:
                logger.error(
                    f"No se encontró el proveedor '{proveedor.nombre}' al intentar marcarlo como procesado."
                )
            except Exception as e:
                logger.error(
                    f"Error al actualizar estado o carteles para proveedor '{proveedor.nombre}'."
                )
                logger.exception(e)

            logger.info(
                f"--- Fin del procesamiento para proveedor: '{proveedor.nombre}' ---"
            )
...
def filtrar_trabajados():
    ...
    try:
        ...
        logger.info(
            f"Proveedores encontrados para exportar: {[p.text_display for p in proveedores]}"
        )  # Asume que text_display existe

        for proveedor in proveedores:
            logger.debug(
                f"Filtrando items trabajados para proveedor: '{proveedor.text_display}'"
            )
            ...
            if not filtrados.exists():
                logger.info(
                    f"No se encontraron items trabajados para '{proveedor.text_display}'."
                )
                continue
            ...
            logger.info(
                f"Exportando {filtrados.count()} items trabajados de '{proveedor.text_display}' a '{filename}'"
            )
            ...
            try:
                ...
                logger.error(
                    f"Error de I/O al escribir el archivo CSV '{filename}' para proveedor '{proveedor.text_display}'."
                )
                logger.exception(e)
            except Exception as e:
                logger.error(
                    f"Error inesperado al escribir el archivo CSV '{filename}' para proveedor '{proveedor.text_display}'."
                )
                logger.exception(e)
...
def asociar_proveedores():
    ...
    try:
        ...
        logger.info(
            f"Items sin proveedor encontrados: {total_items}. Intentando asociar..."
        )
        lista_proveedores_all = (
            ListaProveedores.objects.all()
        )  # Asumiendo que ListaProveedores tiene 'abreviatura' y relaciona a 'Proveedor'
        ...
        for lp in lista_proveedores_all:
            ...
            if hasattr(lp, "proveedor_obj") and lp.proveedor_obj is not None:
                ...
                logger.warning(
                    f"ListaProveedores ID {lp.id} (Abrev: {lp.abreviatura}) no tiene un proveedor_obj asociado."
                )

        logger.debug(
            f"Mapa de abreviaturas a IDs de Proveedor creado: {abreviatura_a_proveedor_id}"
        )
            ...
            for item in batch_items:
                try:
                    ...
                    if len(partes_codigo) > 1:
                        ...
                        proveedor_id_encontrado = abreviatura_a_proveedor_id.get(
                            abreviatura
                        )
                        ...
                        if proveedor_id_encontrado:
                            ...
                            item.proveedor_id = (
                                proveedor_id_encontrado  # Asignar directamente el ID
                            )
                    ...
                    logger.error(
                        f"Error procesando asociación para item código '{item.codigo}'."
                    )
                    logger.exception(e)
                    no_encontrados_count += 1
        ...
        if items_para_actualizar:
            logger.info(
                f"Realizando Bulk Update para asociar {len(items_para_actualizar)} items..."
            )
            try:
                Item.objects.bulk_update(
                    items_para_actualizar, ["proveedor"], batch_size=batch_size
                )
                logger.info(f"Bulk Update de asociación completado.")
            except Exception as e:
                logger.error(
                    "Error durante el Bulk Update de asociación de proveedores."
                )
                logger.exception(e)
        else:
        ...
        logger.info(
            f"Asociación finalizada. Items asociados: {asociados_count}. Items no asociados/con error: {no_encontrados_count}"
        )
