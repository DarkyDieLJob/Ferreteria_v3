...
class EditarPedidoView(GeneralPedidoView):
    ...
    def get(self, request, pedido_id=0, proveedor_id=0):
        ...
        if pedido_id != 0:
            ...
            context["Lista_articulos_pedidos"] = ArticuloPedido.objects.filter(
                pedido=pedido_id
            )
        elif proveedor_id != 0:
            context["proveedor"] = Proveedor.objects.get(id=proveedor_id)
            context["pedido"] = (
                Pedido.objects.filter(proveedor=proveedor_id)
                .exclude(estado="Et")
                .first()
            )
            context["Lista_articulos_pedidos"] = ArticuloPedido.objects.filter(
    ...
    def post(self, request, pedido_id=0, proveedor_id=0):
        ...
        if form.is_valid():
            ...
            articulo_en_lista_pedido = Lista_Pedidos.objects.get_or_create(
                proveedor=pedido.proveedor, item=form.cleaned_data["item"]
            )
...
def actualizar_cantidad(request, articulo_id):
    ...
    if articulo_pedido.cantidad > cantidad:
        ...
        articulo_faltante.cantidad = articulo_faltante.cantidad - (
...
def agregar_al_pedido(request):
    ...
    articulo_faltante = Lista_Pedidos.objects.get(
        proveedor_id=data.get("proveedor_id"), item_id=data.get("item_id")
    )
    articulo_faltante.pedido = True
    articulo_faltante.cantidad = articulo_faltante.cantidad - float(
        data.get("cantidad")
    )
    articulo_faltante.save()
...
def enviar_pedido(request):
    ...
    if not Pedido.objects.get(id=data.get("pedido_id")).articulo_pedido.all():
        return JsonResponse(
            {"status": "error", "message": "El pedido no tiene artículos"}
        )
    else:
