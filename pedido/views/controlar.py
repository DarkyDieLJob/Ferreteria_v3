...
class ControlarPedidoView(GeneralPedidoView):
    ...
    def post(self, request):
        ...
        context["lista_articulo_que_no_llegaron"] = ArticuloPedido.objects.filter(
            pedido=pedido_id, llego=False
        )
        pedido, _ = Pedido.objects.get_or_create(
            proveedor=pedido_controlado.proveedor, estado="Pd"
        )
...
def agregar_devolucion(request):
    ...
    articulo = Lista_Pedidos.objects.get(
...
def actualizar_llego(request, articulo_id):
    ...
    articulo_faltante, _ = Lista_Pedidos.objects.get_or_create(
        proveedor_id=articulo_pedido.proveedor.id, item_id=articulo_pedido.item.id
    )
